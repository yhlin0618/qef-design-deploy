# Create df_segments_by_customer table (DuckDB)
...
create_df_segments_by_customer_table <- function(con, or_replace = TRUE, verbose = FALSE) {
  stopifnot(inherits(con, "DBIConnection"))

  query <- generate_create_table_query(
    con = con,
    target_table = "df_segments_by_customer",
    column_defs = list(
  ...
  if (verbose) {
    message("[DDL] df_segments_by_customer")
    cat(query, "\n")
  }
